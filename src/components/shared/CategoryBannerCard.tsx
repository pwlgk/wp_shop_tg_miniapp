...
export const CategoryBannerCard = ({ category }: CategoryBannerCardProps) => {
  return (
    <Link to={`/catalog/${category.id}`} className="block group">
      <div className="relative overflow-hidden rounded-3xl shadow-sm transition-all duration-300 group-hover:shadow-lg">
        <AspectRatio ratio={16 / 8} className="bg-muted">
          ...
          <div className="absolute top-4 right-4 h-8 w-8 rounded-full bg-white/20 backdrop-blur-sm flex items-center justify-center text-white opacity-0 group-hover:opacity-100 transition-opacity">
